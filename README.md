# TaofeiAI Workbench — AI 智能体工作台

基于 **taofei_api + FastAPI + Web 前端** 的最小可运行桌面应用项目，支持打包为单个 exe。
...
| AI 智能体 | **taofei_api** | Agent / Crew / Process / Task 多智能体协作框架 |
| LLM 兼容层 | **LangChain**（langchain-openai / langchain-core） | taofei_api.LLM 缺失时基于 ChatOpenAI 的兼容适配器 |
...
taofei_app/
├── backend/
│   ├── main.py          # FastAPI 后端：taofei_api 封装 + 任务 API + 静态页面
...
├── build/
│   └── TaofeiAPI.spec   # PyInstaller 打包配置
...
.venv\Scripts\python.exe -m PyInstaller --noconfirm build\TaofeiAPI.spec
```
...
**分发使用：**
1. 将 `dist\TaofeiAPI.exe` 复制到目标机器（无需安装 Python）
2. 在目标目录放置 `.env`（含 `DEEPSEEK_API_KEY=sk-xxx`）
3. 双击 `TaofeiAPI.exe`，浏览器自动打开工作台
...
2. **[1/4]** PyInstaller 收集 `backend/` + `frontend/` → `dist/TaofeiAPI.exe`
...
**架构说明**：
1. Electron 启动时 spawn 后端（开发模式用 `.venv\Scripts\python.exe backend\main.py --no-browser`；生产模式用 `process.resourcesPath\backend\TaofeiAPI.exe`）
